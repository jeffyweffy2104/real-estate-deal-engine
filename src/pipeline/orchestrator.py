...
from src.pipeline.stages import NeighborhoodProvider, model_stage, normalize_stage
...
def run_pipeline(config: dict, provider: IngestionProvider, neighborhood_profiles: dict | None = None) -> tuple[list, RunContext, dict]:
    ...
    norm_sales = normalize_stage(raw_sales, ctx)
    ...
    ctx.record_stage("normalize_rents", len(raw_rents), len(norm_rents), len(raw_rents) - len(norm_rents))

    provider_obj = NeighborhoodProvider(neighborhood_profiles or {}, allow_fallback=True)
    final_records = model_stage(norm_sales, norm_rents, config, ctx, provider_obj)
