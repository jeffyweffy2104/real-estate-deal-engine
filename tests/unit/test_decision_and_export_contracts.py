...
def test_neighborhood_fallback_is_explicitly_labeled():
    ...
    assert records[0].neighborhood_confidence < 0.9


def test_exporters_write_stable_canonical_columns(tmp_path):
    ...
    with open(json_path, encoding="utf-8") as f:
        payload = json.load(f)
        assert payload
        assert list(payload[0].keys()) == sorted(CSV_COLUMNS)
